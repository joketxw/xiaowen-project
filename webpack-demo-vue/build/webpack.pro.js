...
module.exports={
    ...
    output: {
        ...
        publicPath: "./",
        filename: "static/js/[name].[chunkhash].bundle.js",  //chunkhash只有被修改了的文件的文件名，hash值修改   hash每次修改任何一个文件，所有文件名的hash至都将改变
        chunkFilename: 'static/js/[name].[chunkhash].chunk.js',//chunkFilename路由蓝加载时用到
    ...
    resolve: {
        alias: {
            '@':path.join(__dirname,'..','src/page/index')
        }
    },
    devServer: {
        host:'localhost',
        ...
        rules: [
            ...
            },
            {
                ...
                use: {
                    loader: 'babel-loader',
                    options: {
                        presets: ['@babel/preset-env'], // 根据目标浏览器自动转换为相应es5代码   cnpm i babel-loader @babel/core @babel/preset-env --save-dev
                        plugins:['@babel/plugin-syntax-dynamic-import']             // cnpm i @babel/plugin-syntax-dynamic-import --save-dev
                    }
                }
            },
        ]
    ...
    plugins: [
        new HtmlWebpackPlugin({
            ...
            chunks: ['index']//允许插入到模板中的一些chunk，不配置此项默认会将entry中所有的thunk注入到模板中。在配置多个页面时，每个页面注入的thunk应该是不相同的，需要通过该配置为不同页面注入不同的thunk；
